...
I have visually compared the GADM and natural earth products and can conclusively state that the GADM product more accurately maps coastal areas. 
...
lakes <- rnaturalearth::ne_download(
  category = 'physical', type = 'lakes_north_america', scale = 10) |>
  select(name) |>
  sf::st_make_valid() 

lakes <- bind_rows(greatlakes, lakes) |>
  sf::st_make_valid() 

# one lake is irredeemable and inconsequential for our purposes 
lakes <- filter(lakes, st_is_valid(lakes))
lakes <- lakes |>
  st_union() |>
  st_make_valid()
...
st_write(us, file.path(p, 'gadm41_USA-land.gpkg'))

rm(greatlakes, minorIslands, p, us, lakes)
```
