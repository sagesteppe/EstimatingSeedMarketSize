...
This set will be used when we need to make area calculations across large areas, e.g. from Puero Rico to Alaska. 
...
invalidCast <- st_cast(invalid, 'POLYGON')

castval <- sf::st_is_valid(invalidCast)

# we will show here that these are weirdos; somehow polygons but not anything there
# we will just disregard them. 
bad_pieces <- invalid[ which(castval==FALSE), ]
st_area(bad_pieces)

# knowing that the areas covered by these bad polygons sum to < 1m^2 we can just 
# dump them. Let's recombine all of the good polygons back into a multipolygon. 

now_valid <- invalidCast |>
  filter(st_is_valid(invalidCast))

# one of these groups is real bad... We need to proceed piece wise

units <- split(now_valid, f = now_valid$Unit_Nm)

# ah ha ! here it is- the problem is the refuge that crosses the antimeridian!
# unsure how it hasn't triggered an issue before now. 

# let's use st_split on this puppy first to ensure any island on the anti get's
# sliced in two and reconstituted.

antimeridian <- antimeridian |> 
  sf::st_transform(st_crs(padus))

proj2geoid_antimerid <- function(x){

  tryCatch(
    expr = {
          pieces <- lwgeom::st_split(x, antimeridian)
          whole <- summarize(pieces, geom = st_union(geom))
          whole <- st_make_valid(whole)
...
# what is wrong with 3 and 7? 7 CONFIRMED WEIRDO. 

plot(units[[7]])

units1 <- lapply(units[8], proj2geoid_antimerid)

# now we can just recombine these fixed geometries back onto our data. 

fixed <- bind_cols(
  st_drop_geometry(invalid), 
  geom = invalidCast
) |>
  st_as_sf()
