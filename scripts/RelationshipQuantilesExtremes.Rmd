...
all_quants <- function(x){
  ...
  approx(c(0, x[['Tau']]), c(0, x[['Prediction']]), xout = des_res)
...
outtie <- burnedAreasSimulator(historic = ak, extremes = frid, sims = 20, years = 5)
...
ggplot(data = dd, aes(x = Year, y= value, color = name, group = name))+ 
  geom_point() + 
  geom_line()
