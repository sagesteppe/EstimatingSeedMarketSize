...
outtie <- burnedAreasSimulator(historic = ak, extremes = frid, sims = 10, years = 3)
...
bas <- function(sims, years){
  ...
  pb <- txtProgressBar(
   ...
   parallel::clusterExport(
    ...
    foreach::foreach(j = seq_along(1:sims), .packages = 'quantregGrowth') %dopar% {
      
      x <- historic # overwrite the last set of simulations results 
      for (i in seq_along(1:years)){ 
        message(sims[j], years[i])
      }
