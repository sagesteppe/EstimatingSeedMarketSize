...
dat_new <- lapply(X = split(qfd, f = qfd$Region), FUN = reconcileEVT_quantiles, ev = frid, thresh = 0.025) |>
  bind_rows()
...
all_quants <- function(x){
  ...
  tmp <- splinefun(x = c(0, x[['Tau']]), y = c(0, x[['Prediction']]), method="hyman")
  
  dt <- data.frame(
    'Tau' = des_res,
    'Prediction' = approx(c(0, x[['Tau']]), c(0, x[['Prediction']]), xout = des_res)$y,
   # 'Prediction' = tmp(x = c(0, x[['Tau']])),
    'Method' = 'Quantile'
  ) |> 
    dplyr::bind_rows(x) |>
    dplyr::arrange(Tau)
...
sspl <- smooth.spline(c(0, x[['Tau']]), c(0, x[['Prediction']]))
tmp <- splinefun(x = c(0, x[['Tau']]), y = c(0, x[['Prediction']]), method="hyman")

plot(c(0, x[['Tau']]), c(0, x[['Prediction']]), col = 'grey80')
lines(sspl,col="blue")
lines(c(0, x[['Tau']]),  tmp(x = c(0, x[['Tau']])), col = 'red')
...
v <- splinefun(c(0, x[['Prediction']]), method = 'hyman')

library(scam)
fit = scam(c(0, x[['Prediction']]) ~ s(1:100, k=10, bs="mpi", m=5), family=gaussian(link="identity"))
...
bnch <- bench::mark(
  outtie <- BurnedAreasSimulator(
    historic = ak, extremes = frid, sims = 50, years = 5, recalc_extremes = 'every') # 'every is failing when many
)
